...
def load_and_prepare_image_experimental(img_path, max_resolution, enable_timing=True):
    """🧪 EXPERIMENTAL: Even more aggressive optimizations"""
    start_time = time.time()
    if enable_timing:
        debug_print(f"📸 YOLO: Loading and preparing image (EXPERIMENTAL): {img_path}")
    ...
    resize_start = time.time()
    # 🧪 EXPERIMENT 1: Try fastest resize method
    img_resized_bgr = cv2.resize(img_bgr, (target_w, target_h), interpolation=cv2.INTER_NEAREST)
    resize_time = time.time() - resize_start
    
    convert_start = time.time()
    # 🧪 EXPERIMENT 2: Manual BGR->RGB + normalize in single loop (potentially faster)
    img_resized_rgb = img_resized_bgr[:, :, ::-1]  # BGR->RGB slice (very fast)
    
    # 🧪 EXPERIMENT 3: Pre-allocate output tensor
    input_tensor = np.empty((1, 3, target_h, target_w), dtype=np.float32)
    
    # 🧪 EXPERIMENT 4: Direct copy with normalization to final tensor shape
    for c in range(3):
        input_tensor[0, c, :, :] = img_resized_rgb[:, :, c].astype(np.float32, copy=False) * np.float32(1.0/255.0)
    ...
    if enable_timing:
        total_time = time.time() - start_time
        debug_print(f"  YOLO image preparation (EXPERIMENTAL): {total_time:.3f}s")
        debug_print(f"   - Loading: {load_time:.3f}s")
        debug_print(f"   - Resizing: {resize_time:.3f}s")
        debug_print(f"   - Array conversion: {convert_time:.3f}s")
...
def run_batch_inference_optimized(model_path, input_tensors, enable_timing=True):
    """
    🚀 NEW: Batch inference for multiple images
    """
    start_time = time.time()
    if enable_timing:
        debug_print(f"🧠 YOLO: Running BATCH inference ({len(input_tensors)} images)...")
    ...
    input_dict = {input_name: batch_tensor}
    prep_time = time.time() - prep_start
    ...
    if enable_timing:
        total_time = time.time() - start_time
        debug_print(f"  YOLO BATCH timing breakdown:")
        debug_print(f"   - Cache/session access: {cache_time:.3f}s")
        debug_print(f"   - Batch preparation: {prep_time:.3f}s")
        debug_print(f"   - Batch inference: {inference_time:.3f}s")
        debug_print(f"   - Total: {total_time:.3f}s")
        debug_print(f"   - Per image: {total_time/len(input_tensors):.3f}s")
...
class YOLODetector:
    ...
    def detect_batch(self, image_paths: List[str]) -> List[List[Dict[str, Any]]]:
        """
        🚀 NEW: Efficient batch processing for multiple images
        """
        if not image_paths:
            return []
        ...
        if self.config.enable_timing:
            debug_print(f"\n🚀 Starting YOLO BATCH detection pipeline...")
            debug_print(f"  Images: {len(image_paths)} files")
            debug_print(f"🤖 Model: {self.config.model_path}")
            if self.config.enable_content_filtering:
                debug_print(f"🚀 Content filtering: ENABLED (min pixels: {self.config.min_content_pixels})")
            debug_print("=" * 60)
        ...
        for img_path in image_paths:
            try:
                input_tensor, input_size, orig_size, scaling_factors, content_image = load_and_prepare_image_ultra_fast(
                    img_path, self.config.max_resolution, enable_timing=False
                )
                input_tensors.append(input_tensor)
                content_images.append(content_image)
                image_metas.append((input_size, orig_size, scaling_factors))
            except Exception as e:
                debug_print(f"⚠️  Error loading {img_path}: {e}")
                input_tensors.append(None)
                content_images.append(None)
                image_metas.append(None)
        ...
        # Batch inference
        valid_tensors = [t for t in input_tensors if t is not None]
        if not valid_tensors:
            debug_print("❌ No valid images to process")
            return []
        
        output = run_batch_inference_optimized(
            self.config.model_path, valid_tensors, self.config.enable_timing
        )
        ...
        for i, (img_path, meta) in enumerate(zip(image_paths, image_metas)):
            if meta is None:
                all_detections.append([])
                continue
                
            input_size, orig_size, scaling_factors = meta
            
            boxes_raw = postprocess_optimized(
                output, input_size, orig_size, scaling_factors,
                self.config.conf_threshold, self.config.iou_threshold,
                enable_timing=False, enable_debug=self.config.enable_debug,
                batch_idx=valid_idx
            )
            
            # Convert to standardized format
            detections = []
            for j, box in enumerate(boxes_raw):
                x1, y1, x2, y2 = box
                clipped_bbox = self.clip_bbox_to_image_bounds([x1, y1, x2, y2], orig_size[0], orig_size[1])
                if clipped_bbox:
                    detections.append({
            ...
            # Content filtering
            if self.config.enable_content_filtering and len(detections) > 0 and content_images[i] is not None:
                detections, _ = filter_sparse_boxes_ultra_fast(
                    content_images[i], detections, self.config.min_content_pixels
                )
        ...
        if self.config.enable_timing:
            ...
            debug_print("=" * 60)
            debug_print(f"  🚀 BATCH Pipeline completed in {total_time:.3f}s")
            debug_print(f"  Average per image: {total_time/len(image_paths):.3f}s")
            debug_print(f"  Final result: {total_detections} total detections across {len(image_paths)} images")
