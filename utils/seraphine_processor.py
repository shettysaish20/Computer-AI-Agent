...
class BBoxProcessor:
    ...
    def scale_bbox_for_display(self, bbox: BBox) -> Tuple[int, int]:
        """Scale bbox for display according to rules"""
        ...
        if is_long_box:
            ...
            # Scale up if too small (only for non-long boxes)
            if width < self.MIN_DIMENSION:
                scale_factor = self.MIN_DIMENSION / width
                width = self.MIN_DIMENSION
                height = int(height * scale_factor)
                self.log(f"RESIZE: BBox ID {bbox.merged_id} width scaled up from {original_width} to {width} (factor: {scale_factor:.3f})")
            
            if height < self.MIN_DIMENSION:
                scale_factor = self.MIN_DIMENSION / height
                height = self.MIN_DIMENSION
                width = int(width * scale_factor)
                self.log(f"RESIZE: BBox ID {bbox.merged_id} height scaled up from {original_height} to {height} (factor: {scale_factor:.3f})")
        ...
        if width != original_width or height != original_height:
            self.log(f"RESIZE: Final dimensions for BBox ID {bbox.merged_id}: {original_width}x{original_height} -> {width}x{height}")
    ...
    def _generate_combined_group_images(self, groups: Dict[str, List[BBox]], base_name: str, 
        ...
        for group_id, boxes in ordered_groups:
            ...
            for i, (bbox, width, height, label, cropped_image, label_width) in enumerate(scaled_boxes):
                # Calculate positions
                image_y = current_y
                label_y = current_y - self.LABEL_TOP_PADDING + 5  # Position label above image
                ...
                draw.rectangle([current_x, image_y, current_x + width, image_y + height], 
                             outline='grey', width=self.BBOX_BORDER_WIDTH)
    ...
    def generate_images(self, output_dir: str = "outputs", return_images: bool = False):
        ...
        Args:
            output_dir: Directory to save images (still saves files)
            return_images: If True, also returns PIL images directly
            
        Returns:
            If return_images=True: Dict with 'image_count' and 'generated_images' list
            If return_images=False: None (original behavior)
        """
        self.log("Steps 10-11: Generating images with combined H and V groups")
        
        os.makedirs(output_dir, exist_ok=True)
        
        # Combine all groups (both horizontal and vertical)
        all_groups = self.final_groups
        
        self.log(f"Processing {len(all_groups)} total groups (H and V combined)")
        
        if return_images:
            # Generate combined images and return them
            image_count, generated_images = self._generate_combined_group_images_with_return(
            ...
            image_count = self._generate_combined_group_images(all_groups, "combined_groups", output_dir, 0)
            self.log(f"Generated {image_count} combined images")
            return None

    def _generate_combined_group_images_with_return(self, groups: Dict[str, List[BBox]], base_name: str, 
                                                   output_dir: str, start_image_count: int) -> Tuple[int, List[Tuple]]:
        """Generate images and return them directly along with saving - FIXED VERSION"""
        image_count = start_image_count
        current_y = self.PADDING + self.LABEL_TOP_PADDING
        current_image = Image.new('RGB', (self.IMAGE_WIDTH, self.IMAGE_HEIGHT), 'white')
...
class FinalSeraphineProcessor:
    ...
    def _convert_to_old_group_format(self, group_type: str) -> List[List[Dict]]:
        ...
        for group_id, bbox_list in groups_dict.items():
            ...
            for bbox in bbox_list:
                ...
                old_group.append(old_detection)
    ...
    def _create_group_dict(self) -> Dict[str, List[Dict]]:
        ...
        for group_id, bbox_list in self.bbox_processor.final_groups.items():
            ...
            for bbox in bbox_list:
                ...
                group_dict[group_id].append(old_detection)
    ...
    def _create_compatible_analysis(self, detections: List[Dict]) -> Dict[str, Any]:
        ...
        # Create group_details for compatibility
        group_details = {}
        for group_id, bbox_list in self.bbox_processor.final_groups.items():
            # Convert bbox objects to dict format for bboxes list
            bboxes = []
            for bbox in bbox_list:
                bboxes.append({
                    'bbox': [bbox.x1, bbox.y1, bbox.x2, bbox.y2],
...
def convert_detections_to_seraphine_format(detections: List[Dict]) -> List[Dict]:
    """
    Convert detections to format expected by FinalSeraphineProcessor
    Maintains compatibility with old conversion function
    
    Args:
        detections: List of detection dictionaries
        
    Returns:
        List of detections in seraphine format
    """
    ...
    for detection in detections:
        ...
        if 'bbox' in detection:
            ...
            bbox = [
                ...
                detection.get('x2', detection.get('right', 0)),
                detection.get('y2', detection.get('bottom', 0))
            ]
        
        seraphine_detection = {
            'bbox': bbox,
            'id': detection.get('id', detection.get('detection_id', len(seraphine_detections))),
            'merged_id': detection.get('merged_id', detection.get('id', len(seraphine_detections))),
            'type': detection.get('type', detection.get('class', 'unknown')),
